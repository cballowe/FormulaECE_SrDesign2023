...
/*Some timing stuff...making asyncronous timer capabilities*/
...
void AutoCodeTopLevel(void)
{
    ...
    struct timespec startControlTimer, finishControlTimer, deltaControlTimer;
    struct timespec startPozyxTimer, finishPozyxTimer, deltaPozyxTimer;
    struct timespec startPWMSendTimer, finishPWMSendTimer, deltaPWMSendTimer;
    deltaPWMSendTimer.tv_nsec = 0.0;
    int iteration = 0;
    float extraTime = 0.0;
    ...
    sprintf(outbuf, "%d,%d",72,72);
    writeOutPipe(pip, outbuf, BUFFSIZE);

    clock_gettime(CLOCK_REALTIME, &startControlTimer);
    ...
    while (1) {
        ...
        /* global */
        /* X = X_dot * dt + X */
        clock_gettime(CLOCK_REALTIME, &startPozyxTimer);
        readInPipe(pip, buf, BUFFSIZE);
        if(buf[0] != 0x0)
        {
            sscanf(strtok(buf, ","), "%lf", &posx);
        ...
        clock_gettime(CLOCK_REALTIME, &finishPozyxTimer);
        sub_timespec(startPozyxTimer, finishPozyxTimer, &deltaPozyxTimer);
        printf("Read-in data time: %d.%.9ld, ", (int)deltaPozyxTimer.tv_sec, deltaPozyxTimer.tv_nsec);

        AutoCodeEncoder(U, encoderData);
        //AutoCodeUpdatePosPozyx(X, pozyxInfo, counter);

        clock_gettime(CLOCK_REALTIME, &finishControlTimer);
        sub_timespec(startControlTimer, finishControlTimer, &deltaControlTimer);

        while(deltaControlTimer.tv_nsec < (1000*1000*100 - deltaPozyxTimer.tv_nsec - deltaPWMSendTimer.tv_nsec)) // 100 for 100 milliseconds
        {
            clock_gettime(CLOCK_REALTIME, &finishControlTimer);
            sub_timespec(startControlTimer, finishControlTimer, &deltaControlTimer);
        }

        printf("Control loop time: %d.%.9ld, ", (int)deltaControlTimer.tv_sec, deltaControlTimer.tv_nsec);
        printf("Total time: %.9ld, ", deltaPozyxTimer.tv_nsec+deltaControlTimer.tv_nsec+deltaPWMSendTimer.tv_nsec);

        /* For timing testing
        iteration++;
        extraTime += ((deltaPozyxTimer.tv_nsec+deltaControlTimer.tv_nsec+deltaPWMSendTimer.tv_nsec) - 100000000);
        
        if(iteration == 600){
            printf("%.9f",extraTime/pow(10,9));
            break;
        }
        */
        ...
        clock_gettime(CLOCK_REALTIME, &startControlTimer); // reset timer

        //wait(1); // wait 0.1 seconds
        clock_gettime(CLOCK_REALTIME, &startPWMSendTimer);
        sprintf(outbuf, "%d,%d",PWM[0],PWM[1]);
        writeOutPipe(pip, outbuf, BUFFSIZE);
        clock_gettime(CLOCK_REALTIME, &finishPWMSendTimer);
        sub_timespec(startPWMSendTimer, finishPWMSendTimer, &deltaPWMSendTimer);
        printf("Write time: %d.%.9ld\n", (int)deltaPWMSendTimer.tv_sec, deltaPWMSendTimer.tv_nsec);
    }
}
...
void AutoCodeInitialize(double X[3], double U[2])
{
  ...
  X[1] = 11; // Testing! Ypos: 11 for circ/halftrack, 8.5 for line; 8ish for track
